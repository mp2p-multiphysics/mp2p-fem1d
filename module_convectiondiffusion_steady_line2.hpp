#ifndef MODULE_CONVECTIONDIFFUSION_UNSTABILIZED_STEADY_LINE2
#define MODULE_CONVECTIONDIFFUSION_UNSTABILIZED_STEADY_LINE2
#include <vector>
#include "Eigen/Eigen"
...
class ConvectionDiffusionSteadyLine2Class
{
    ...
    // constructor
    ConvectionDiffusionSteadyLine2Class(GridLine2Struct &gl2s_in, BoundaryLine2Struct &bl2s_in, IntegralLine2Class &il2c_in)
    {
...
void ConvectionDiffusionSteadyLine2Class::matrix_fill
(
    Eigen::SparseMatrix<double> &a_mat, Eigen::VectorXd &b_vec, Eigen::VectorXd &x_last_iteration_vec,
    ScalarLine2Class &diffusion_coefficient_sl2c, ScalarLine2Class &velocity_sl2c, ScalarLine2Class &species_generation_sl2c,
    int start_id
)
{
    ...
    for (int m = 0; m < gl2s.num_element; m++)
    {
        ...
        for (int i = 0; i < 2; i++){
        for (int j = 0; j < 2; j++){
            a_mat.coeffRef(n_arr[i], n_arr[j]) += diffcoeff_arr[i]*il2c.integral_div_Ni_line2_dot_div_Nj_line2_vec[m][i][j] + v_arr[i]*il2c.integral_Ni_line2_derivative_Nj_line2_x_vec[m][i][j];
        }}

        // calculate b_vec coefficients
        for (int i = 0; i < 2; i++)
        {
            b_vec.coeffRef(n_arr[i]) += specgen_arr[i]*il2c.integral_Ni_line2_vec[m][i];
        }
