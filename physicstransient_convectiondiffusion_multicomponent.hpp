...
class PhysicsTransientConvectionDiffusionMulticomponent : public PhysicsTransientBase
{
  /*
    ...
    a * du/dt = -div(-b * grad(u)) - v * grad(u) + c wherein:

    a1 * d(u1)/dt = -div(-b11 * grad(u1) - b12 * grad(u2) - b13 * grad(u3) - ...) - v * grad(u1) + c1
    a2 * d(u2)/dt = -div(-b21 * grad(u1) - b22 * grad(u2) - b23 * grad(u3) - ...) - v * grad(u2) + c2
    a3 * d(u3)/dt = -div(-b31 * grad(u1) - b32 * grad(u2) - b33 * grad(u3) - ...) - v * grad(u3) + c3
    ...
