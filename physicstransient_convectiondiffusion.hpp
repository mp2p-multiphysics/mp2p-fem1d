...
)
{
    ...
    */
    ...
    for (int indx_d = 0; indx_d < domain_group_ptr->domain_l2_ptr_vec.size(); indx_d++)
    {
        ...
        matrix_fill_domain(a_mat, c_mat, d_vec, x_vec, x_last_timestep_vec, dt, domain_ptr, boundary_ptr, integral_ptr, derivativecoefficient_ptr, diffusioncoefficient_ptr, velocity_x_ptr, generationcoefficient_ptr);
...
)
{
    ...
    for (int edid = 0; edid < domain_ptr->num_element_domain; edid++)
    {
        ...
        double dervcoeff_p0 = diffusioncoefficient_ptr->point_value_vec[p0_pdid];
        double dervcoeff_p1 = diffusioncoefficient_ptr->point_value_vec[p1_pdid];
        ...
        double specgen_p0 = generationcoefficient_ptr->point_value_vec[p0_pdid];
        double specgen_p1 = generationcoefficient_ptr->point_value_vec[p1_pdid];
        ...
        {
            int mat_row = start_row + pfid_arr[indx_i];
            d_vec.coeffRef(mat_row) += specgen_arr[indx_i]*integral_ptr->integral_Ni_vec[edid][indx_i];
        }
